...
public class PsController: MonoBehaviour
{
    ...
    void Start()
    {
        ...
        startpos = new Vector3(-0.39f, 1.78f, -1.6f);
    ...
    void PlayerMoveAndRotation()
    {
        ...
        Vector3 movInput = new Vector3(InputX, 0, InputZ);
        if (!frozenGround)
        { 
        // Movement
        ...
        if (  movInput.z > 0 || movInput.x >0 || movInput.x < 0 || movInput.z <0 )
        {
            anim.SetFloat("Speed", Mathf.Abs(movInput.x) + Mathf.Abs(movInput.z));
...
// remove all the status when the player dies 
    void initPlayer()
    {
        frozenGround = false;
        Speed = 300;
    ...
    IEnumerator IceballCD()
    {
        ...
        IceBallP1 script = iceball.GetComponent<IceBallP1>();
    ...
    IEnumerator FireBallCooldown()
    {
        ...
        if (!playerHuge)
        {
           GameObject fireball = Instantiate(fireBall, spellInitPostion.transform.position, transform.rotation);
            FireBallP1 FBscript = fireBall.GetComponent<FireBallP1>();
